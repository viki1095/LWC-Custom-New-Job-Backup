...
import fetchHost from '@salesforce/schema/Job__c.Host__c';
import fetchTemplate from '@salesforce/schema/Job__c.Buytoyota_Template__c';

import JobObject from '@salesforce/schema/Job__c'; 
import fetchTags from '@salesforce/schema/Job__c.Tags_Needed__c';

import saveNewMicrositeJob from '@salesforce/apex/CreateJobWizardController.saveJob';



export default class CreateNewMicrositeJob extends NavigationMixin(LightningElement) {
    ...
    // Get Object Info.
    @wire (getObjectInfo, {objectApiName: JobObject})
    accountObjectInfo;

    // Get Picklist values.
    @wire(getPicklistValues, {recordTypeId: '012000000000000AAA', fieldApiName: fetchTags })
    ...
    handleChange(event) {
        ...
        else if (field === 'AccompanyingMedia') {
            this.selectedAccompanyingMedia  = event.target.checked;
        }
        else if (field === 'CustomURL') {
            this.selectedCustomURL = event.target.checked;
        }
        else if (field === 'URLPurchase') {
            this.selectedURLPurchase  = event.target.checked;
        }
...
*/
        ...
        if(allValid1 && all_Compobox_Valid){
            ...
            saveNewMicrositeJob({ newJob: jobRecordInfo }).then(accountId => {
                this.dispatchEvent(
                    new ShowToastEvent({
                        title: 'Success',
    ...
    createJobRecord() {
        ...
        //  4 - Get current form lightning-dual-listbox fields
        //     Loop the input list and get every input value and assign it to the desired field
        //     The field name is set on every input dataset as the attribute "field"
        let inputs5 = this.template.querySelectorAll('lightning-dual-listbox');
        inputs5.forEach(input => {
            jobRecord[input.dataset.fieldName] = input.value;
        });
        
        jobRecord.Accompanying_Media_Campa__c  = this.selectedAccompanyingMedia;
        jobRecord.Custom_URL_Needed__c  = this.selectedCustomURL;
        jobRecord.URL_Purchase_Needed__c  = this.selectedURLPurchase;

        //NEW - Interactive RT 
        jobRecord.RecordTypeId ='012800000003WkFAAU';
        jobRecord.Type__c = 'Microsite';
        return jobRecord;
    }
